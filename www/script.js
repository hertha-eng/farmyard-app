...
let hasNativeOAuthListener = false;
...
function registerNativeOAuthListener(){
    if (hasNativeOAuthListener || !supabaseClient || !isNativeCapacitorRuntime()) return;
    const AppPlugin = getCapacitorPlugin('App');
    if (!AppPlugin?.addListener) return;

    AppPlugin.addListener('appUrlOpen', async ({ url }) => {
        if (!url || !url.startsWith(MOBILE_OAUTH_REDIRECT_URI)) return;
        ...
        try {
            ...
            console.error('Failed to complete native OAuth sign-in', error);
            showToast(error.message || 'Could not finish Google sign-in');
        }
    });
...
async function signInWithGoogle(){
    ...
    if (isNativeCapacitorRuntime()) {
        ...
        if (!BrowserPlugin?.open) {
            showToast('Google sign-in needs the Capacitor Browser plugin in the mobile build');
            return;
        }
        ...
        await BrowserPlugin.open({ url: data.url, windowName: '_self' });
        return;
    }
